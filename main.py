...
async def generate_story_summary_with_llm(story_json: str) -> str:
    ...
    try:
        ...
        # 스토리 요약을 위한 프롬프트 생성
        summary_prompt = f"""당신은 투자교육 게임 스토리 분석 전문가입니다.
...
좋은 예시:
"중위험 종목은 초반 급등 후 조정, 고위험 종목은 큰 등락을 거쳐 회복하며 끝나 분산보다 고위험 중심의 기회 포착 전략이 유리한 시나리오입니다."
"저위험 종목은 안정적 상승, 고위험 종목은 후반 급등으로 적극적 리스크 테이킹이 높은 수익을 가져다준 변동성 활용 시나리오입니다."
